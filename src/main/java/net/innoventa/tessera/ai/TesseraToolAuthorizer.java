...
/**
 * What an action costs, asked of the authorization model Tessera already has.
 *
 * <p><strong>This is the case {@code ToolAuthorizer} exists as a seam for.</strong> Tessera does not
 * use {@code jmouse-access}: it has its own model — a role per project, with per-member ALLOW/DENY
 * overrides applied on top, deny winning. The library knows none of that and does not need to. It asks
 * one question twice and this class answers it out of whatever the product happens to have.
 *
...
public class TesseraToolAuthorizer implements ToolAuthorizer {
    ...
    public boolean permits(CallerIdentity caller, ToolAction action) {
        return memberships.findByMemberId(caller.callerId()).stream()
                ...
                .distinct()
                .anyMatch(projectId ->
                        permissions.hasPermission(caller.callerId(), projectId, action.requiredPermission()));
    }

    @Override
    public boolean permitsInScope(CallerIdentity caller, ToolAction action, InvocationScope scope) {
        return permissions.hasPermission(caller.callerId(), scope.id(), action.requiredPermission());
    }
}
