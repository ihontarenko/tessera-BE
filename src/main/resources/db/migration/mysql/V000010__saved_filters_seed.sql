...
--  point of a preset вЂ” "my open issues" means the same thing in every project and
...
--  SavedFilterPresetTest, which parses THIS FILE rather than a copy вЂ” a preset that
...
--    * `(вЂ¦ in [вЂ¦])` keeps its brackets вЂ” jME binds `in` LOOSER than `and`, so the
--      unbracketed form parses as `name in ([вЂ¦] and вЂ¦)` and quietly answers false.
